...
<link rel="stylesheet" href="{{ asset('css/stok/stok_edit.css') }}">
...
<div class="stok-form-page">
    <div class="stok-form-header">
        ...
        <div>
            <h1 class="form-title">Edit Stok Bahan</h1>
            <p class="form-subtitle">Perbarui data bahan baku dan jumlah stok produksi</p>
        </div>
    </div>
        ...
        <form action="{{ route('stok.update', $stok->id_stok) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select name="kategori_bahan_id" id="kategori_bahan_id" class="form-control">
                    ...
                    @foreach($kategori as $k)
                        <option value="{{ $k->id }}" {{ old('kategori_bahan_id', $stok->kategori_bahan_id) == $k->id ? 'selected' : '' }}>
                            {{ $k->nama_kategori }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input
                    ...
                    value="{{ old('detail_bahan', $stok->detail_bahan) }}"
                >
                @error('detail_bahan')
                    <small class="text-error">{{ $message }}</small>
                @enderror
            ...
            @if($stok->gambar)
                <div class="form-group">
                    ...
                    <div class="preview-image-wrapper">
                        <img src="{{ asset('storage/' . $stok->gambar) }}" alt="{{ $stok->detail_bahan }}" class="preview-image">
                    </div>
                </div>
            @endif
